...
import Zero from './wizard/ZeroForm.vue'
import fs from 'fs'
export default {
  name: 'orderWizard',
  // 注册子组件
  ...
  methods: {
    ...
    async exportExcel() {
      ...
      console.log('xlsxStyle', this.$xlsxStyle)
      ...
      const ws_data = [
        ...
        [
          {
            v: `ADDRESS: ${this.contract.sellerAddress}`,
            t: 's',
            s: { font, alignment: { wrap_text: 1, horizontal: 'top', vertical: 'top', indent: 0 } }
          }
        ],
        [
          {
            v: `TEL: ${this.contract.sellerTEL} FAX: ${this.contract.sellerFAX}`,
            t: 's',
            s: { font }
          }
        ],
        ...
        [
          {
            v: `ADDRESS: ${this.contract.buyerAddress}`,
            t: 's',
            s: { font }
          }
        ],
        [
          {
            v: `TEL: ${this.contract.buyerTEL} FAX: ${this.contract.buyerFAX}`,
            t: 's',
            s: { font }
          }
        ],
      ...
      for (const product of this.contract.offerSheet.products) {
        if (product.name.length > 0) {
          ws_data.push([{ v: product.name, t: 's', s: { font } }])
          i += 1
        }
        for (const quotation of product.quotations) {
          ws_data.push(
            [
              { v: quotation.size, t: 's', s: { font } },
              { v: quotation.quantity, t: 's', s: { font } },
              { v: quotation.price, t: 's', s: { font } },
              { v: quotation.total, t: 's', s: { font } }
            ]
          )
          i += 1
        }
      ...
      for (const attach of this.contract.offerSheet.attachs) {
        ws_data.push(
          [
            { v: attach.title, t: 's', s: { font } },
            '',
            '',
            { v: attach.amount, t: 's', s: { font } }
          ]
        )
        i += 1
      }
      ...
      ws_data.push(
        [
          { v: 'TOTAL:', t: 's', s: { font } },
          { v: this.contract.offerSheet.count, t: 's', s: { font } },
          '',
          { v: `USD ${this.contract.offerSheet.sum}`, t: 's', s: { font } }
        ]
      )
      // 添加合同条款
      ws_data.push([{ v: OrderInfo.Terms[2], t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[3] + this.contract.packaging, t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[4], t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[5] + this.contract.loading, t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[6] + this.contract.destination, t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[7] + this.contract.payment, t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[8], t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[9], t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[10], t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[11], t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[12], t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[13], t: 's', s: { font } }])
      ws_data.push([{ v: OrderInfo.Terms[14], t: 's', s: { font } }])
      ws_data.push([''])
      ws_data.push([{ v: OrderInfo.Terms[15], t: 's', s: { font } }, '', { v: OrderInfo.Terms[16], t: 's', s: { font } }, ''])
      ws_data.push([''])
      ws_data.push([''])
      const merge = [
        { s: { r: 0, c: 0 }, e: { r: 1, c: 3 } }, // 合并标题
        { s: { r: 2, c: 0 }, e: { r: 2, c: 1 } }, // 合并卖方公司名称
        { s: { r: 3, c: 0 }, e: { r: 4, c: 1 } }, // 合并卖方公司地址
      ...
      const tmp = this.$xlsxStyle.write(wb, { bookType: 'xlsx', bookSST: false, type: 'binary', cellStyles: true })
      this.$fileSaver.saveAs(new Blob([s2ab(tmp)], { type: 'application/octet-stream' }), 'abc.xlsx');
      console.log(wb)
    },
  },
  mounted() {
